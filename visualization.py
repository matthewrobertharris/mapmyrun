...
def visualize_road_network(G, output_file='road_network.html'):
    """Visualize the basic road network before classification.
    ...
    """
    # Log the input data
    logging.info(f"Visualizing road network with {len(G.edges())} edges")
    
    # Create a map centered on the graph
    center_lat = np.mean([G.nodes[node]['y'] for node in G.nodes()])
    center_lon = np.mean([G.nodes[node]['x'] for node in G.nodes()])
    logging.info(f"Map center: ({center_lat}, {center_lon})")
    
    m = folium.Map(location=[center_lat, center_lon], zoom_start=14)
    
    # Create feature group for all roads
    roads_group = folium.FeatureGroup(name='Road Network')
    
    # Add all edges
    for u, v, data in G.edges(data=True):
        # Get the full geometry of the edge if available
        if 'geometry' in data:
        ...
        popup_text = f"<b>Road:</b> {road_name}<br>"
        popup_text += f"<b>Edge ID:</b> {u}->{v}"
    ...
    <div style="position: fixed; 
        ...
        <h4 style="margin-top:0">Road Network</h4>
        <p><i class="fa fa-square" style="color:blue"></i> Roads</p>
        <p>Total edges: {}</p>
    </div>
    '''.format(len(G.edges()))
    m.get_root().html.add_child(folium.Element(legend_html))
    ...
    # Save the map
    m.save(output_file)
    logging.info(f"Road network visualization saved to {output_file}")
    logging.info(f"Total edges visualized: {len(G.edges())}")

def visualize_classification(G, classification, output_file='road_classification.html'):
    """Visualize the classification of road segments."""
    # Log the input data
    logging.info(f"Visualizing classification with:")
    logging.info(f"  Total edges: {len(classification['run_edges']) + len(classification['not_run_edges'])}")
    logging.info(f"  Run edges: {len(classification['run_edges'])}")
    logging.info(f"  Not run edges: {len(classification['not_run_edges'])}")
    
    try:
        # Create a map centered on the graph
        valid_nodes = [node for node in G.nodes() if 'y' in G.nodes[node] and 'x' in G.nodes[node]]
        if not valid_nodes:
            raise ValueError("No nodes with valid coordinates found in the graph")
        
        center_lat = np.mean([G.nodes[node]['y'] for node in valid_nodes])
        center_lon = np.mean([G.nodes[node]['x'] for node in valid_nodes])
        logging.info(f"Map center: ({center_lat}, {center_lon})")
        
        m = folium.Map(location=[center_lat, center_lon], zoom_start=14)
        
        # Create feature groups for better layer management
        run_edges_group = folium.FeatureGroup(name='Run Edges')
        not_run_edges_group = folium.FeatureGroup(name='Not Run Edges')
        
        # Function to safely process an edge
        def process_edge(edge, edge_type='run'):
            try:
                # Get the full geometry of the edge if available
                edge_data = G.get_edge_data(edge[0], edge[1])
                if not edge_data:
                    logging.warning(f"No edge data found for edge {edge}")
                    return
                ...
                if 'geometry' in edge_data[0]:
                    ...
                    # Check if nodes have required coordinates
                    if not all(coord in G.nodes[edge[0]] for coord in ['x', 'y']) or \
                       not all(coord in G.nodes[edge[1]] for coord in ['x', 'y']):
                        logging.warning(f"Missing coordinates for edge {edge}")
                        return
                ...
                # Get road name with segment number from classification
                road_name = classification.get('road_names', {}).get(edge, 'Unnamed Road')
                
                # Create popup with edge information
                popup_text = f"<b>Road Segment:</b> {road_name}<br>"
                popup_text += f"<b>Edge ID:</b> {edge[0]}->{edge[1]}<br>"
                if 'deviation' in classification and edge in classification['deviation']:
                    popup_text += f"<b>Avg Deviation:</b> {classification['deviation'][edge]:.1f}m<br>"
                if 'max_deviation' in classification and edge in classification['max_deviation']:
                    popup_text += f"<b>Max Deviation:</b> {classification['max_deviation'][edge]:.1f}m"
                
                # Create the line
                line = folium.PolyLine(
                    locations=coords,
                    color='green' if edge_type == 'run' else 'red',
                    weight=3 if edge_type == 'run' else 2,
                    opacity=0.7 if edge_type == 'run' else 0.5,
                    popup=folium.Popup(popup_text, max_width=300)
                )
                
                if edge_type == 'run':
                    line.add_to(run_edges_group)
                else:
                    line.add_to(not_run_edges_group)
                
                logging.debug(f"Added {edge_type} edge: {edge} ({road_name}) with {len(coords)} points")
                return True
            
            except Exception as e:
                logging.error(f"Error processing {edge_type} edge {edge}: {str(e)}")
                return False
        
        # Process run edges
        successful_run_edges = 0
        for edge in classification['run_edges']:
            if process_edge(edge, 'run'):
                successful_run_edges += 1
        
        # Process not run edges
        successful_not_run_edges = 0
        for edge in classification['not_run_edges']:
            if process_edge(edge, 'not_run'):
                successful_not_run_edges += 1
        
        # Add feature groups to map
        run_edges_group.add_to(m)
        not_run_edges_group.add_to(m)
        ...
        <div style="position: fixed; 
                    ...
                    border:2px solid grey; z-index:9999; background-color:white;
                    padding: 10px; font-size: 14px;">
            <h4 style="margin-top:0">Road Classification</h4>
            <p><i class="fa fa-square" style="color:green"></i> Run segments</p>
            <p><i class="fa fa-square" style="color:red"></i> Not run segments</p>
            <p>Total edges: {}</p>
            <p>Run edges: {} ({:.1f}%)</p>
        </div>
        '''.format(
            successful_run_edges + successful_not_run_edges,
            successful_run_edges,
            (successful_run_edges / (successful_run_edges + successful_not_run_edges) * 100) if (successful_run_edges + successful_not_run_edges) > 0 else 0
        )
        m.get_root().html.add_child(folium.Element(legend_html))
        ...
        logging.info(f"Successfully visualized edges:")
        logging.info(f"  - Run edges: {successful_run_edges}/{len(classification['run_edges'])}")
        logging.info(f"  - Not run edges: {successful_not_run_edges}/{len(classification['not_run_edges'])}")
        ...
        if os.path.exists(output_file):
            ...
            if file_size < 1000:  # If file is suspiciously small
                logging.warning("Output file appears to be too small. There might be an issue with the visualization.")
        else:
